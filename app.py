...
def render_main_interface():
    ...
    @media (max-width: 768px) {{
        ...
        #main-animated-title-container {{ 
            width: 100%; height: 8vw; /* Tăng chiều cao để chứa font lớn hơn */
            overflow: hidden; 
            white-space: nowrap;
        }}
        
        /* SỬA LỖI 2 & 3: Tăng kích thước chữ, số 1 cân đối, VÀ chạy nhanh hơn */
        #main-animated-title-container h1 {{ 
            font-size: 5.5vw; /* To hơn (từ 4.5vw lên 5.5vw) */
            letter-spacing: 3px; /* Giảm nhẹ để số 1 cân đối hơn */
            padding: 0 10px; 
            white-space: nowrap; 
            
            animation: colorShift 10s ease infinite, scrollText 12s linear infinite; /* Chạy nhanh hơn (từ 20s xuống 12s) */
            
            display: inline-block; 
            text-align: center;
        }}

        /* Tiêu đề 2: TRA CỨU PART NUMBER dịch xuống dưới */
        #sub-static-title h2 {{ 
            font-size: 4.5vw; 
            margin-top: 60px; 
            margin-bottom: 20px;
